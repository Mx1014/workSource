...
/**
 * 在这里写这个可以覆盖库中的同名类，并用自定义类加载器加载这个类， 原因是为了修改 xss 过滤里面对 css 的过滤逻辑
 *
 * @author xq.tian
 */
    ...
    public String lexicalValueToString(LexicalUnit lu) {
        switch (lu.getLexicalUnitType()) {
            ...
            case LexicalUnit.SAC_FUNCTION:
                sb = new StringBuffer(lu.getFunctionName()+"(");
                param = lu.getParameters();
                if (param != null) {
                    sb.append(lexicalValueToString(param)); // R value

                    while (param.getNextLexicalUnit() != null) {
                        sb.append(lexicalValueToString(param.getNextLexicalUnit()));
