...
//	}
	...
	public RestResponse findTargetByNameAndAddress(FindTargetByNameAndAddressCommand cmd) {
		RestResponse resp = new RestResponse(userService.findTargetByNameAndAddress(cmd.getContractNum(),cmd.getTargetName(),cmd.getBuildingName(),cmd.getApartmentName(),cmd.getCommunityId(),cmd.getTel()));
		resp.setErrorCode(ErrorCodes.SUCCESS);
		resp.setErrorDescription("OK");
		return resp;
	}
