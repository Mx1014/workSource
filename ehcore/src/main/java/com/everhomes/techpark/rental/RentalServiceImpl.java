...
import com.everhomes.techpark.punch.PunchGeopoint;
...
//		rentalRule.setRefundFlag(cmd.getRefundFlag);
	...
	@Override
	public void addRentalSite(
			AddRentalSiteCommand cmd) {
...
//		TODO : siteItem.setPrice(cmd.getItemPrice());
	...
	public void addRentalSiteRules(AddRentalSiteRulesCommand cmd) { 
		...
		if(cmd.getRentalType().equals(RentalType.DAY.getCode())){
			rsr.setStepLengthDay(Integer.valueOf(cmd.getStepLength()));
		}
		else if(cmd.getRentalType().equals(RentalType.HOUR.getCode())){
			rsr.setStepLengthTime(Time.valueOf(cmd.getStepLength()));
		}
		if (null != cmd.getBeginTime())
		rsr.setBeginTime(Time.valueOf(cmd.getBeginTime()));
...
//		TODO: rsr.setPrice(cmd.getPrice());
	...
	public FindRentalSitesCommandResponse findRentalSites(
		...
		Long userId = UserContext.current().getUser().getId();
		checkEnterpriseCommunityIdIsNull(cmd.getEnterpriseCommunityId());
		List<RentalSite> rentalSites = rentalProvider.findRentalSites(cmd.getEnterpriseCommunityId(),cmd.getSiteType());
...
//	TODO:		rSiteDTO.setSiteType(rentalSite.getSiteType());
	...
	public FindRentalSiteRulesCommandResponse findRentalSiteRules(
		...
		for (RentalSiteRule rsr :rsrs){
			...
			if(rsr.getRentalType().equals(RentalType.DAY.getCode()))
				dto.setStepLength(timeSF.format(rsr.getStepLengthTime().getTime()));
			...
			if(rsr.getLoopType().equals(LoopType.EVERYDAY.getCode())){
				...
				dto.setRuleDate(String.valueOf(calendar.get(Calendar.DAY_OF_WEEK)));
