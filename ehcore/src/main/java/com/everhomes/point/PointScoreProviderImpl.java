...
import com.everhomes.server.schema.tables.records.EhPointScoresRecord;
...
public class PointScoreProviderImpl implements PointScoreProvider {
    ...
    public <T> T findUserPointScore(Integer namespaceId, Long systemId, Long uid, Class<T> clazz) {
        ...
        SelectQuery<EhPointScoresRecord> query = context().selectFrom(t).getQuery();
        if (namespaceId != null) {
        ...
        if (uid != null) {
            query.addConditions(t.USER_ID.eq(uid));
        }
        query.addOrderBy(t.ID.desc());
        return query.fetchAnyInto(clazz);
    }
