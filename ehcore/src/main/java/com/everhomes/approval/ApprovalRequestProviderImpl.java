    ...
    public Double countOvertimeDurationByUserAndMonth(Long userId, String ownerType, Long ownerId,
        ...
        try {
            ...
            SelectConditionStep<Record1<BigDecimal>> step = getReadOnlyContext()
                    .select(Tables.EH_PUNCH_EXCEPTION_REQUESTS.DURATION_DAY.sum().round(3)).from(Tables.EH_PUNCH_EXCEPTION_REQUESTS)
...
//			LOGGER.debug(step.toString());
    ...
    @Override
    public int countAbnormalUserRequest(Long creatorId, Long ownerId, java.util.Date start, java.util.Date end) {

        SelectConditionStep<Record1<Integer>> step = getReadOnlyContext()
                .select(Tables.EH_PUNCH_EXCEPTION_REQUESTS.ID.count()).from(Tables.EH_PUNCH_EXCEPTION_REQUESTS)
                .where(Tables.EH_PUNCH_EXCEPTION_REQUESTS.CREATOR_UID.eq(creatorId))
                .and(Tables.EH_PUNCH_EXCEPTION_REQUESTS.ENTERPRISE_ID.eq(ownerId))
                .and(Tables.EH_PUNCH_EXCEPTION_REQUESTS.CREATE_TIME.greaterOrEqual(new Timestamp(start.getTime())))
                .and(Tables.EH_PUNCH_EXCEPTION_REQUESTS.CREATE_TIME.lt(new Timestamp(end.getTime()))
