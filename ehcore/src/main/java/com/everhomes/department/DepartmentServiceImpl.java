...
import com.everhomes.pm.AssginPmTopicCommand;
...
import com.everhomes.pm.PmMemberTargetType;
import com.everhomes.pm.PmTaskStatus;
import com.everhomes.pm.PmTaskType;
import com.everhomes.pm.PropertyMgrProvider;
import com.everhomes.pm.PropertyMgrService;
import com.everhomes.pm.PropertyServiceErrorCode;
import com.everhomes.pm.QueryPropTopicByCategoryCommand;
import com.everhomes.pm.SetPmTopicStatusCommand;
...
public class DepartmentServiceImpl implements DepartmentService {
    ...
    public ListDepartmentsCommandResponse listDepartments(ListDepartmentsCommand cmd) {
    	...
    	int totalCount = departmentProvider.countDepartments(cmd.getName());
    	if(totalCount == 0) return response;
    	int pageSize = PaginationConfigHelper.getPageSize(configurationProvider, cmd.getPageSize());
    	int pageCount = getPageCount(totalCount, pageSize);
    	List<Department> result = departmentProvider.listDepartments(cmd.getName(), cmd.getPageOffset(), pageSize);
    ...
    @Override
    public ListDptMemberCommandResponse getUserOwningDepartments() {
    	User user  = UserContext.current().getUser();
    	//权限控制
    	ListDptMemberCommandResponse response = new ListDptMemberCommandResponse();
    	List<DepartmentMember> reslut = departmentProvider.listDepartmentMembers (user.getId());
    	response.setMembers(reslut.stream()
    			.map(r->{ return ConvertHelper.convert(r,DepartmentMemberDTO.class); })
                .collect(Collectors.toList()));
    	return response;
    }
    ...
    @Override
    public ListDptMemberCommandResponse listDptMembers(ListDptMemberCommand cmd) {
    	User user  = UserContext.current().getUser();
    	//权限控制
    	ListDptMemberCommandResponse response = new ListDptMemberCommandResponse();
    	cmd.setPageOffset(cmd.getPageOffset() == null ? 1: cmd.getPageOffset());
    	int totalCount = departmentProvider.countDepartmentMembers(cmd.getDepartmentId(), null);
    	if(totalCount == 0) return response;
    	int pageSize = PaginationConfigHelper.getPageSize(configurationProvider, cmd.getPageSize());
    	int pageCount = getPageCount(totalCount, pageSize);
    	List<DepartmentMember> reslut = departmentProvider.listDepartmentMembers(cmd.getDepartmentId(), null, cmd.getPageOffset(), pageSize);
    	response.setMembers(reslut.stream()
    			.map(r->{ return ConvertHelper.convert(r,DepartmentMemberDTO.class); })
                .collect(Collectors.toList()));
    	response.setNextPageOffset(cmd.getPageOffset()==pageCount? null : cmd.getPageOffset()+1);
