...
System.out.println();
	...
	public List<OrganizationMemberDTO> convertDTO(List<OrganizationMember> organizationMembers, Organization org){
		...
		return organizationMembers.stream().map((c) ->{
			...
			//职级
			dto.setJobLevels(this.getOrganizationMemberGroups(OrganizationGroupType.JOB_LEVEL, dto.getContactToken(),org.getPath()));
    ...
    @Override
    public List<CommunityOrganizationTreeDTO> listCommunityOrganizationTree(ListCommunityOrganizationTreeCommand cmd) {
        ...
        if (communityDTOs != null && communityDTOs.size() > 0) {
            for (CommunityDTO communityDTO : communityDTOs) {
                ...
                treeDTO.setCommunity(communityDTO);
                ...
                if (enterprisesResponse != null && enterprisesResponse.getDtos() != null) {
                    treeDTO.setOrganizationList(enterprisesResponse.getDtos());
                }
...
//			}
	...
	public ListOrganizationContactCommandResponse listOrganizationContacts(ListOrganizationContactCommand cmd) {
		...
		List<OrganizationMember> organizationMembers = this.organizationProvider.listOrganizationPersonnels(cmd.getKeywords(),orgCommoand, cmd.getIsSignedup(),visibleFlag, locator, pageSize);
