...
//                ConvertHelper.convert(organizationMember, OrganizationMemberDTO.class);
        ...
        if (cmd.getEmployeeNo() != null)
            organizationMemberDetails.setEmployeeNo(cmd.getEmployeeNo());
...
//        organizationMember.setMemberGroup(OrganizationMemberGroupType.MANAGER.getCode());
        ...
        dbProvider.execute((TransactionStatus status) -> {
            ...
            // 先把把成员从公司所有机构都删除掉
            List<OrganizationMember> members = this.organizationProvider.listOrganizationMemberByPath(org.getPath(), groupTypes, cmd.getContactToken());
            for (OrganizationMember member : members) {
                if (!enterpriseIds.contains(member.getOrganizationId())) {
                    //记录 退出的公司
                    if (OrganizationGroupType.ENTERPRISE == OrganizationGroupType.fromCode(member.getGroupType())) {
