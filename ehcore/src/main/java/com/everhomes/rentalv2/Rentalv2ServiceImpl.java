...
//			rentalProvider.createRentalSiteRule(rsr);
	...
	public void cancelRentalBill(CancelRentalBillCommand cmd) {
		...
		dbProvider.execute((TransactionStatus status) -> {
			...
			//用户积分
			if (!order.getRentalUid().equals(0L))
	...
	public void cancelOrderSendMessage(RentalOrder rentalBill) {
		...
		managerContent.append(rentalBill.getUserName());
	...
	private AddRentalBillItemCommandResponse actualAddRentalItemBill(AddRentalBillItemCommand cmd, ActivityRosterPayVersionFlag version) {
		...
		this.dbProvider.execute((TransactionStatus status) -> {
			...
			if (bill.getPayMode().equals(PayMode.ONLINE_PAY.getCode()) || bill.getSource().equals(RentalBillSource.BACK_GROUND.getCode())) {
				...
				if (!bill.getRentalUid().equals(0L)) {
					if (null != flowCase) {
...
//	    	LOGGER.debug("cmd = \n"+cmd);
	...
	private Long createDoorAuth(Integer namespaceId,Long userId, String phone,String userName,Long timeBegin, Long timeEnd, Long doorId, Long authUserId) {
		if (userId != null && !userId.equals(0L)) {
			...
			cmd.setValidFromMs(timeBegin);
			cmd.setValidEndMs(timeEnd);
			DoorAuthDTO dto = doorAccessService.createLocalVisitorAuth(cmd);
			return dto.getId();
		}
	}
...
//		response.setSitePics(convertRentalSitePicDTOs(pics));
	...
	@Override
	public FindRentalSiteDayStatusCommandResponse findRentalSiteDayStatus(FindRentalSiteDayStatusCommand cmd) {
		if (StringUtils.isBlank(cmd.getResourceType())) {
			cmd.setResourceType(RentalV2ResourceType.DEFAULT.getCode());
		}
		...
		processCells(rs, cmd.getRentalType());
		FindRentalSiteDayStatusCommandResponse response = ConvertHelper.convert(rule, FindRentalSiteDayStatusCommandResponse.class);
		response.setRentalSiteId(rs.getId());
		...
		List<RentalSiteDayRulesDTO> siteDays = new ArrayList<>();
		processDayRuleDTOs(start, end, siteDays, rs, rule,  cmd.getRentalType(),
				cmd.getPackageName());
		response.setResourceCounts(rs.getResourceCounts());
		response.setSiteDay(siteDays.get(0));
