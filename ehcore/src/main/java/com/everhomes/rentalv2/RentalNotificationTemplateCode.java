...
public interface RentalNotificationTemplateCode {
    ...
    int RENTAL_PAY_SUCCESS_CODE = 5; // 支付成功短信：【正中会】您已完成支付，成功预约{使用时间}的{资源名称}，请按照预约的时段使用资源，感谢您的使用。
    ...
    int RENTAL_USER_CANCEL_ORDER = 13;  //用户取消订单推送消息    订单取消通知：您的${VIP车位预约}订单已成功取消。
