...
@Component
public class HaianThirdServiceImpl implements HaianThirdService {
    private static final Logger LOGGER = LoggerFactory.getLogger(HaianThirdServiceImpl.class);
    ...
    @Override
    public EncryptForInsurobotDTO encryptForInsurobot(EncryptForInsurobotCommand cmd) {
    	
    	EncryptForInsurobotDTO dto = new EncryptForInsurobotDTO();
    	...
    	sb.append(InsurobotConstant.XIANLUOBO_URL).append("?")
    		...
    		.append("&carNo=").append(StringUtils.isBlank(cmd.getCarNo())?"":cmd.getCarNo());
	...
	private String stringMD5(String pw) {
		try {  
			...
			LOGGER.error("Xianluobo encrypt error");
