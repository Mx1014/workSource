...
import com.everhomes.rest.investment.CustomerTrackerType;
...
@Component(DynamicExcelStrings.DYNAMIC_EXCEL_HANDLER + DynamicExcelStrings.INVITED_CUSTOMER)

public class InvitedCustomerDynamicExcelHandler implements DynamicExcelHandler {
    private static final org.slf4j.Logger LOGGER = LoggerFactory.getLogger(InvitedCustomerDynamicExcelHandler.class);
    ...
    private int importCustomerInfo(ImportFieldExcelCommand customerInfo, ImportFileResultLog<Map<String, String>> importLogs, int failedNumber, List<DynamicColumnDTO> columns, String sheetName) {
        ...
        String customerContactString = "";
        String channelContactString = "";
        String trackerUidString = "";
            ...
            columnLoop:  for (DynamicColumnDTO column : columns) {
                ...
                try {
                    if (!"channelContact".equals(column.getFieldName())
                        ...
                        if ("channelContact".equals(column.getFieldName())) {
                            channelContactString = column.getValue();
                        }
                        if ("trackerUid".equals(column.getFieldName())) {
                            trackerUidString = column.getValue();
                        }
                        ...
                        if(StringUtils.isNotBlank(customerContactString)){
                            dealString = customerContactString;
                        }
                        if(StringUtils.isNotBlank(channelContactString)){
                            dealString = channelContactString;
                        }
                        if(StringUtils.isNotBlank(trackerUidString)){
                            dealString = trackerUidString;
                        }
...
//                        boolean dealResult = dealCustomerAdminsAndAddress(customerAddressString, customerAdminString, importLogs, enterpriseCustomer, column, originColumns, sheetName);
    ...
    private boolean dealDynamicItemsAndTrackingUidField(ImportFieldExcelCommand customerInfo, ImportFileResultLog<Map<String, String>> importLogs, List<DynamicColumnDTO> columns, EnterpriseCustomer enterpriseCustomer, DynamicColumnDTO column,String sheetName ) {
        ...
        if("categoryItemId".equals(column.getFieldName()) || "levelItemId".equals(column.getFieldName())
            ...
            if(item != null) {
                ...
                if("levelItemId".equals(column.getFieldName())){
                    if(item.getItemId() != null){
                        if(item.getItemId() == CustomerLevelType.REGISTERED_CUSTOMER.getCode()){
                            enterpriseCustomer.setCustomerSource(InvitedCustomerType.ENTEPRIRSE_CUSTOMER.getCode());
                        }else{
                            enterpriseCustomer.setCustomerSource(InvitedCustomerType.INVITED_CUSTOMER.getCode());
                        }
                    }
                }
            }else {
    ...
    private boolean dealDynamicTrackerUidAndContactField(ImportFieldExcelCommand customerInfo, String dealString, ImportFileResultLog<Map<String, String>> importLogs, EnterpriseCustomer enterpriseCustomer, List<DynamicColumnDTO> columns, DynamicColumnDTO column, String sheetName){
        if (StringUtils.isNotBlank(dealString)) {

            Boolean isAdmin = checkCustomerAdmin(customerInfo.getOrgId(), null, customerInfo.getNamespaceId());
            String[] userInfos = null;
            List<String> trackerUids = new ArrayList<>();
            if(isAdmin) {
                try {
                    String regex = "^((?:(?!([,|()])).)*\\(\\d+\\),)*(?:(?!([,|()])).)*\\(\\d+\\)";
                    if (!Pattern.matches(regex, dealString)) {
                        throw new Exception("customer enterprise admins format error");
                    }
                    dealString = dealString.replaceAll("\n", "");
                    userInfos = dealString.split(",");
    ...
    private void compareCustomerFieldValues(EnterpriseCustomer exist, EnterpriseCustomer enterpriseCustomer)  {
        ...
        java.lang.reflect.Field[] fields = clz.getSuperclass().getDeclaredFields();
        if (fields.length > 0) {
    ...
    private void createCustomerTracker(EnterpriseCustomer enterpriseCustomer, String customerTrackerString){
        String[] userInfos = null;
        if(StringUtils.isNotBlank(customerTrackerString)){
            ...
            if (userInfos.length > 0) {
                for (String userInfosString : userInfos) {
                    ...
                    String contactPhone = userInfosString.substring(userInfosString.indexOf("(") + 1, userInfosString.lastIndexOf(")"));
                    List<User> users = null;
                    users = userProvider.listUserByKeyword(contactPhone, enterpriseCustomer.getNamespaceId(), new CrossShardListingLocator(), 2);

                    if (users != null && users.size() > 0) {
                        CustomerTracker tracker = new CustomerTracker();
                        tracker.setTrackerUid((users.get(0).getId()));
                        if(enterpriseCustomer.getCustomerSource().equals(InvitedCustomerType.INVITED_CUSTOMER.getCode())){
                            tracker.setTrackerType(CustomerTrackerType.INVITED_TRACKER.getCode());
                        } else if (enterpriseCustomer.getCustomerSource().equals(InvitedCustomerType.ENTEPRIRSE_CUSTOMER.getCode())) {
                            tracker.setTrackerType(CustomerTrackerType.ENTERPRISE_TRACKER.getCode());
                        }
                        tracker.setCommunityId(enterpriseCustomer.getCommunityId());
                        tracker.setNamespaceId(enterpriseCustomer.getNamespaceId());
    ...
    private void createCustomerContact(EnterpriseCustomer enterpriseCustomer, String customerContactString){
        String[] userInfos = null;
        if(StringUtils.isNotBlank(customerContactString)){
    ...
    private void createChannelContact(EnterpriseCustomer enterpriseCustomer, String channelCustomerString){
        String[] userInfos = null;
            ...
            switch(type){
                ...
                case "Timestamp":
                    ...
                    if (pattern1.matcher(value.toString()).matches()) {
                        ...
                        val = new Timestamp(java.sql.Date.valueOf(LocalDate.parse(val.toString(), formatter3)).getTime());
