...
@Component(CustomRequestHandler.CUSTOM_REQUEST_OBJ_RESOLVER_PREFIX + CustomRequestConstants.GOLF_REQUEST_CUSTOM)
public class GolfCustomRequestHandler implements CustomRequestHandler {
...
private static final Logger LOGGER=LoggerFactory.getLogger(ApartmentCustomRequestHandler.class);
	...
	public Long addCustomRequest(AddRequestCommand cmd) {
		ServiceAllianceGolfRequest request = GsonUtil.fromJson(cmd.getRequestJson(), ServiceAllianceGolfRequest.class);
		...
		Long id = yellowPageProvider.createGolfRequest(request);
...
//			}
	...
	private String getNote(ServiceAllianceGolfRequest request) {
	...
	public GetRequestInfoResponse getCustomRequestInfo(Long id) {
		ServiceAllianceGolfRequest request = yellowPageProvider.findGolfRequest(id);
	...
	public List<RequestFieldDTO> toFieldDTOList(Object requestObject) {
		ServiceAllianceGolfRequest field = (ServiceAllianceGolfRequest)requestObject;
		...
		if(template != null && template.getDtos() != null && template.getDtos().size() > 0) {
			EhServiceAllianceGolfRequests request = ConvertHelper.convert(field, EhServiceAllianceGolfRequests.class);
