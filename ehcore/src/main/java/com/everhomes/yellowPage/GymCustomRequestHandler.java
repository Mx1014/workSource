...
@Component(CustomRequestHandler.CUSTOM_REQUEST_OBJ_RESOLVER_PREFIX + CustomRequestConstants.GYM_REQUEST_CUSTOM)
public class GymCustomRequestHandler implements CustomRequestHandler {
...
private static final Logger LOGGER=LoggerFactory.getLogger(ApartmentCustomRequestHandler.class);
	...
	public Long addCustomRequest(AddRequestCommand cmd) {
		...
		Long id = yellowPageProvider.createGymRequest(request);
...
//			}
	...
	private String getNote(ServiceAllianceGymRequest request) {
	...
	public GetRequestInfoResponse getCustomRequestInfo(Long id) {
		ServiceAllianceGymRequest request = yellowPageProvider.findGymRequest(id);
	...
	public List<RequestFieldDTO> toFieldDTOList(Object requestObject) {
		...
		if(template != null && template.getDtos() != null && template.getDtos().size() > 0) {
			EhServiceAllianceGymRequests request = ConvertHelper.convert(field, EhServiceAllianceGymRequests.class);
