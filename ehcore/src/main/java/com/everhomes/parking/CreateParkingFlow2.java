...
public class CreateParkingFlow2 {
    @Autowired
    ...
    private Integer namespaceId =999983;
    private Long moduleId = 40800l;
    private Long orgId = 1008900L;
	private Long owenrId = 10006L;
	...
	public void createFlow1() {
		...
		flowCmd.setStringTag1("3");
    	Long projectId = 240111044331055940L;
		...
		buttonCmd12.setNeedProcessor((byte)0);
		buttonCmd12.setNeedSubject((byte)0);
    ...
    public FlowNodeDTO createFlowNode(CreateFlowNodeCommand cmd) {
     ...
     private void updateNodeReminder(FlowNodeDTO dto, Long orgId) {
     	...
     	FlowActionInfo action = createActionInfo("申请成功，请耐心等候审核资料", orgId);
     	remindCmd.setMessageAction(action);
     	action = createActionInfo("申请成功，请耐心等候审核资料", orgId);
     ...
     private void updateNodeReminder2(FlowNodeDTO dto, Long orgId) {
     	UpdateFlowNodeReminderCommand remindCmd = new UpdateFlowNodeReminderCommand();
     	remindCmd.setFlowNodeId(dto.getId());
     	FlowActionInfo action = createActionInfo("排队中，请耐心等候发放月卡资格", orgId);
     	remindCmd.setMessageAction(action);
     	action = createActionInfo("排队中，请耐心等候发放月卡资格", orgId);
     	action.setReminderAfterMinute(0l);
     	action.setReminderTickMinute(5l);
     	remindCmd.setTickMessageAction(action);
     	flowService.updateFlowNodeReminder(remindCmd);
