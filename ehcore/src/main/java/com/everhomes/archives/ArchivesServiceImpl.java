...
//        organizationService.checkOrganizationpPivilege(cmd.getOrganizationId(), PrivilegeConstants.BATCH_EXPORT_PERSON);
    ...
    public String getEmployeeOrganizationNames(Map<Long, String> map) {
        String names = "";
...
/*    public String getEmployeeOrganizationIds(Map<Long, String> map) {
        String ids = "";
        if (map != null && map.size() > 0)
            ids =
        return name;
    ...
    public void dismissArchivesEmployees(DismissArchivesEmployeesCommand cmd) {
        ...
        dbProvider.execute((TransactionStatus status) -> {
            for (Long detailId : cmd.getDetailIds()) {
                ...
                ArchivesDismissEmployees dismissEmployee = convertToDismissEmployees(employee, cmd);
    ...
    private ArchivesDismissEmployees convertToDismissEmployees(OrganizationMemberDetails employee, DismissArchivesEmployeesCommand cmd){
...
/*    private void addSocialSecurityEndMonth(Long detailId, Long organizationId, String endMonth){
    ...
    private List<ImportArchivesEmployeesDTO> handleImportArchivesEmployees(List resultLists, List fields) {
        ...
        for (int i = 1; i < resultLists.size(); i++) {
            ...
            for (int j = 0; j < fields.size(); j++) {
                RowResult r = (RowResult) resultLists.get(i);
                String value = r.getCells().get(GetExcelLetter(j + 1)) != null ? r.getCells().get(GetExcelLetter(j + 1)) : "";
                values.add(value);
            }
    ...
    private void excelSettings(ExcelUtils excelUtils, GeneralFormDTO form) {
        ...
        excelUtils.setTitleRemark("填写须知：\n" +
                ...
                "    9、注意日期格式为 xxxx-xx-xx", (short) 18, (short) 4480);
        excelUtils.setNeedSequenceColumn(false);
    ...
    private <T> boolean checkArchivesCheckInTime(ImportFileResultLog<T> log, T data, String checkInTime) {
        if (StringUtils.isEmpty(checkInTime)) {
            ...
            if (temp == null) {
                ...
                log.setCode(ArchivesServiceErrorCode.ERROR_DATE_WRONG_FORMAT);
                return false;
            } else
                return true;
        }
...
//            result.setEmployeeStatus(detail.getEmployeeStatus());
            ...
            result.setDepartmentIds(detail.getDepartmentIds());
            result.setJobPosition(detail.getJobPosition());
            result.setJobPositionIds(detail.getJobPositionIds());
            result.setJobLevel(detail.getJobLevel());
            result.setJobLevelIds(detail.getJobLevelIds());
