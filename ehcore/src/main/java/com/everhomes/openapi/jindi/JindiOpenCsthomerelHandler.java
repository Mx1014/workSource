...
public class JindiOpenCsthomerelHandler implements JindiOpenHandler {
	...
	public String fetchData(JindiFetchDataCommand cmd) {
		...
		if (cmd.getPageAnchor() != null) {
			List<Organization> thisList = organizationProvider.listCsthomerelByUpdateTimeAndAnchor(cmd.getNamespaceId(), cmd.getTimestamp(), cmd.getPageAnchor(), pageSize+1);
		...
		if (hasMore == null  && resultList.size() <= pageSize) {
			...
			List<Organization> thisList = organizationProvider.listCsthomerelByUpdateTime(cmd.getNamespaceId(), cmd.getTimestamp(), pageSize+1);
