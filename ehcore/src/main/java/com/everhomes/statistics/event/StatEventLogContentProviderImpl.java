...
import com.everhomes.server.schema.tables.daos.EhStatEventLogContentsDao;
...
@Repository
public class StatEventLogContentProviderImpl implements StatEventLogContentProvider {
	...
	public void createStatEventLogContent(StatEventLogContent statEventLogContent) {
		Long id = sequenceProvider.getNextSequence(NameMapper.getSequenceDomainFromTablePojo(EhStatEventLogContents.class));
		...
		DaoHelper.publishDaoAction(DaoAction.CREATE, EhStatEventLogContents.class, id);
	}
	...
	public void updateStatEventLogContent(StatEventLogContent statEventLogContent) {
		...
		DaoHelper.publishDaoAction(DaoAction.MODIFY, EhStatEventLogContents.class, statEventLogContent.getId());
	}
    ...
    @Override
    public List<StatEventLogContent> listEventLogContent(Timestamp minTime, Timestamp maxTime) {
        return context().selectFrom(Tables.EH_STAT_EVENT_LOG_CONTENTS)
                .where(Tables.EH_STAT_EVENT_LOG_CONTENTS.CREATE_TIME.between(minTime, maxTime))
	...
	private EhStatEventLogContentsDao rwDao() {
        DSLContext context = dbProvider.getDslContext(AccessSpec.readWrite());
        return new EhStatEventLogContentsDao(context.configuration());
	}

	private EhStatEventLogContentsDao dao() {
