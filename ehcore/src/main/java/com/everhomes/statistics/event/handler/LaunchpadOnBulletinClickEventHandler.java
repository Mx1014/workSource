 ...
 */
@Component(AbstractStatEventHandler.EVENT_HANDLER_PREFIX + AbstractStatEventHandler.LAUNCHPAD_ON_BULLETIN_CLICK)
public class LaunchpadOnBulletinClickEventHandler extends AbstractStatEventHandler {
    ...
    public List<StatEventStatistic> process(Namespace namespace, StatEvent statEvent, LocalDate statDate) {
        ...
        //         namespace.getId(), statEvent.getEventName(), minTime, maxTime);
        ...
        for (Map.Entry<String, Integer> entry : layoutIdParamValueToCountMap.entrySet()) {
            ...
            Long layoutId = Long.valueOf(entry.getKey());
            ...
            PortalItemGroup portalItemGroup = portalItemGroupProvider.findPortalItemGroup(mapping.getPortalContentId(), Widget.BULLETINS.getCode(), "Default");
