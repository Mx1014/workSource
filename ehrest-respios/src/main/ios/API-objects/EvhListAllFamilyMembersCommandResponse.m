...
@implementation EvhListAllFamilyMembersCommandResponse

+(id) withJsonString: (NSString*) jsonString
{
    id jsonObject = [EvhJsonSerializationHelper fromJsonString:jsonString];
    if(jsonObject != nil) {
        EvhListAllFamilyMembersCommandResponse* obj = [EvhListAllFamilyMembersCommandResponse new];
...
-(void) toJson: (NSMutableDictionary*) jsonObject 
{
    ...
    if(self.requests) {
        ...
        for(EvhFamilyMemberFullDTO* item in self.requests) {
    ...
    if([jsonObject isKindOfClass:[NSDictionary class]]) {
...

        {
            ...
            for(id itemJson in jsonArray) {
                EvhFamilyMemberFullDTO* item = [EvhFamilyMemberFullDTO new];
