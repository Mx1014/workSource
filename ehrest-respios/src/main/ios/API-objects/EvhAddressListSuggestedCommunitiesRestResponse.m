...
@implementation EvhAddressListSuggestedCommunitiesRestResponse

+(id) withJsonString: (NSString*) jsonString
{
    id jsonObject = [EvhJsonSerializationHelper fromJsonString:jsonString];
    if(jsonObject != nil) {
        EvhAddressListSuggestedCommunitiesRestResponse* obj = [EvhAddressListSuggestedCommunitiesRestResponse new];
...
-(void) toJson: (NSMutableDictionary*) jsonObject 
{
    ...
    if(self.response) {
        ...
        for(EvhCommunitySummaryDTO* item in self.response) {
...
-(id<EvhJsonSerializable>) fromJson: (id) jsonObject 
{
    if([jsonObject isKindOfClass:[NSDictionary class]]) {
        ...
        for(NSMutableDictionary* dic in jsonArray) {
            EvhCommunitySummaryDTO* item = [EvhCommunitySummaryDTO new];
            [item fromJson:dic];
