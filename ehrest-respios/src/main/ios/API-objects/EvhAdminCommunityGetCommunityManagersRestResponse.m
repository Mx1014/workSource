...
@implementation EvhAdminCommunityGetCommunityManagersRestResponse

+(id) withJsonString: (NSString*) jsonString
{
    id jsonObject = [EvhJsonSerializationHelper fromJsonString:jsonString];
    if(jsonObject != nil) {
        EvhAdminCommunityGetCommunityManagersRestResponse* obj = [EvhAdminCommunityGetCommunityManagersRestResponse new];
...
-(void) toJson: (NSMutableDictionary*) jsonObject 
{
    ...
    if(self.response) {
        ...
        for(EvhCommunityManagerDTO* item in self.response) {
...
-(id<EvhJsonSerializable>) fromJson: (id) jsonObject 
{
    if([jsonObject isKindOfClass:[NSDictionary class]]) {
        ...
        for(NSMutableDictionary* dic in jsonArray) {
            EvhCommunityManagerDTO* item = [EvhCommunityManagerDTO new];
            [item fromJson:dic];
