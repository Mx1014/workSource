...
@implementation EvhPkgGetUpgradeFileInfoRestResponse

+(id) withJsonString: (NSString*) jsonString
{
    id jsonObject = [EvhJsonSerializationHelper fromJsonString:jsonString];
    if(jsonObject != nil) {
        EvhPkgGetUpgradeFileInfoRestResponse* obj = [EvhPkgGetUpgradeFileInfoRestResponse new];
...
-(void) toJson: (NSMutableDictionary*) jsonObject 
{
    ...
    if(self.response) {
        ...
        for(EvhClientPackageFileDTO* item in self.response) {
...
-(id<EvhJsonSerializable>) fromJson: (id) jsonObject 
{
    if([jsonObject isKindOfClass:[NSDictionary class]]) {
        ...
        for(NSMutableDictionary* dic in jsonArray) {
            EvhClientPackageFileDTO* item = [EvhClientPackageFileDTO new];
            [item fromJson:dic];
