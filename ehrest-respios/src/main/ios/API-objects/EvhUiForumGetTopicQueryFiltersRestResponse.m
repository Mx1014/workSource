...
@implementation EvhUiForumGetTopicQueryFiltersRestResponse

+(id) withJsonString: (NSString*) jsonString
{
    id jsonObject = [EvhJsonSerializationHelper fromJsonString:jsonString];
    if(jsonObject != nil) {
        EvhUiForumGetTopicQueryFiltersRestResponse* obj = [EvhUiForumGetTopicQueryFiltersRestResponse new];
...
-(void) toJson: (NSMutableDictionary*) jsonObject 
{
    ...
    if(self.response) {
        ...
        for(EvhTopicFilterDTO* item in self.response) {
...
-(id<EvhJsonSerializable>) fromJson: (id) jsonObject 
{
    if([jsonObject isKindOfClass:[NSDictionary class]]) {
        ...
        for(NSMutableDictionary* dic in jsonArray) {
            EvhTopicFilterDTO* item = [EvhTopicFilterDTO new];
            [item fromJson:dic];
