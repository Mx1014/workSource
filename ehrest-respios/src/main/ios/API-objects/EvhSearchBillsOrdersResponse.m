...
@implementation EvhSearchBillsOrdersResponse

+(id) withJsonString: (NSString*) jsonString
{
    id jsonObject = [EvhJsonSerializationHelper fromJsonString:jsonString];
    if(jsonObject != nil) {
        EvhSearchBillsOrdersResponse* obj = [EvhSearchBillsOrdersResponse new];
...
-(void) toJson: (NSMutableDictionary*) jsonObject 
{
    ...
    if(self.requests) {
        ...
        for(EvhPmBillsOrdersDTO* item in self.requests) {
    ...
    if([jsonObject isKindOfClass:[NSDictionary class]]) {
...

        {
            ...
            for(id itemJson in jsonArray) {
                EvhPmBillsOrdersDTO* item = [EvhPmBillsOrdersDTO new];
