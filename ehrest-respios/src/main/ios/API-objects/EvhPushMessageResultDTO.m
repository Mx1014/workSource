...
@implementation EvhPushMessageResultDTO

+(id) withJsonString: (NSString*) jsonString
{
    id jsonObject = [EvhJsonSerializationHelper fromJsonString:jsonString];
    if(jsonObject != nil) {
        EvhPushMessageResultDTO* obj = [EvhPushMessageResultDTO new];
...
-(void) toJson: (NSMutableDictionary*) jsonObject 
{
    ...
    if(self.sendTime)
        [jsonObject setObject: self.sendTime forKey: @"sendTime"];
}
...
-(id<EvhJsonSerializable>) fromJson: (id) jsonObject 
{
    if([jsonObject isKindOfClass:[NSDictionary class]]) {
        ...
        self.sendTime = [jsonObject objectForKey: @"sendTime"];
        if(self.sendTime && [self.sendTime isEqual:[NSNull null]])
            self.sendTime = nil;
