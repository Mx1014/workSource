...
@implementation EvhUiForumGetTopicSentScopesRestResponse

+(id) withJsonString: (NSString*) jsonString
{
    id jsonObject = [EvhJsonSerializationHelper fromJsonString:jsonString];
    if(jsonObject != nil) {
        EvhUiForumGetTopicSentScopesRestResponse* obj = [EvhUiForumGetTopicSentScopesRestResponse new];
...
-(void) toJson: (NSMutableDictionary*) jsonObject 
{
    ...
    if(self.response) {
        ...
        for(EvhTopicScopeDTO* item in self.response) {
...
-(id<EvhJsonSerializable>) fromJson: (id) jsonObject 
{
    if([jsonObject isKindOfClass:[NSDictionary class]]) {
        ...
        for(NSMutableDictionary* dic in jsonArray) {
            EvhTopicScopeDTO* item = [EvhTopicScopeDTO new];
            [item fromJson:dic];
