...
@implementation EvhQualityListRecordsByTaskIdRestResponse

+(id) withJsonString: (NSString*) jsonString
{
    id jsonObject = [EvhJsonSerializationHelper fromJsonString:jsonString];
    if(jsonObject != nil) {
        EvhQualityListRecordsByTaskIdRestResponse* obj = [EvhQualityListRecordsByTaskIdRestResponse new];
...
-(void) toJson: (NSMutableDictionary*) jsonObject 
{
    ...
    if(self.response) {
        ...
        for(EvhQualityInspectionTaskRecordsDTO* item in self.response) {
...
-(id<EvhJsonSerializable>) fromJson: (id) jsonObject 
{
    if([jsonObject isKindOfClass:[NSDictionary class]]) {
        ...
        for(NSMutableDictionary* dic in jsonArray) {
            EvhQualityInspectionTaskRecordsDTO* item = [EvhQualityInspectionTaskRecordsDTO new];
            [item fromJson:dic];
