...
#ifdef RTCONFIG_DUALWAN
			if(cfg != SWCFG_BRIDGE){
				int wan1cfg = nvram_get_int("wans_lanport"); //1
...
void init_others(void)
{
	...
	if (model == MODEL_RTAC56U || model == MODEL_RTAC56S || model == MODEL_RTAC3200 || model == MODEL_RTAC68U || model == MODEL_EA6900 || model == MODEL_EA9200 || model == MODEL_R6300V2  || model == MODEL_R7000 || model == MODEL_R8000 || model == MODEL_WS880 || model == MODEL_RPAC68U || model == MODEL_RTAC87U || model == MODEL_RTAC88U || model == MODEL_RTAC3100 || model == MODEL_RTAC5300 || model == MODEL_RTN18U) {
#ifdef SMP
		int fd;
