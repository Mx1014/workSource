...
func proportion(mw *imagick.MagickWand, proportion int, cols uint, rows uint) error {
	...
	if proportion == 0 {
		...
		if cols == 0 || rows == 0 {
			...
			//fmt.Print(fmt.Sprintf("p=1, wi_scale(im, %d, %d)\n", cols, rows))
			result = mw.ResizeImage(cols, rows, imagick.FILTER_UNDEFINED, 1.0)
		} else {
			...
			if colsRate > rowsRate {
				sCols = cols
				sRows = uint(round(colsRate * float64(imRows)))
				y = uint(math.Floor(float64((sRows - rows) / 2.0)))
			} else {
				sCols = uint(round(rowsRate * float64(imCols)))
				sRows = rows
				x = uint(math.Floor(float64((sCols - cols) / 2.0)))
			}

			//fmt.Print(fmt.Sprintf("p=1, x=%d y=%d, imRows=%d imCols=%d sRows=%d sCols=%d \n", x, y, imRows, imCols, sRows, sCols))
...
func convert(mw *imagick.MagickWand, request *ImageRequest) error {
	...
	//fmt.Print(fmt.Sprintf("image cols %d, rows %d \n", cols, rows))
