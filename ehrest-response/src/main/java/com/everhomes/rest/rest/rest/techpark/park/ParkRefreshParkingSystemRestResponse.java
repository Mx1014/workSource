// @formatter:off
// generated file: DO NOT EDIT
package com.everhomes.rest.rest.rest.techpark.park;

import com.everhomes.rest.RestResponseBase;

import com.everhomes.rest.techpark.park.RechargeSuccessResponse;

public class ParkRefreshParkingSystemRestResponse extends RestResponseBase {

    private RechargeSuccessResponse response;

    public ParkRefreshParkingSystemRestResponse () {
    }

    public RechargeSuccessResponse getResponse() {
        return response;
    }

    public void setResponse(RechargeSuccessResponse response) {
        this.response = response;
    }
}
