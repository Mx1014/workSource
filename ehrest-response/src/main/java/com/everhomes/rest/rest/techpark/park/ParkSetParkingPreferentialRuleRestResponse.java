// @formatter:off
// generated file: DO NOT EDIT
package com.everhomes.rest.rest.techpark.park;

import com.everhomes.rest.RestResponseBase;

import com.everhomes.rest.techpark.park.ParkingPreferentialRuleDTO;

public class ParkSetParkingPreferentialRuleRestResponse extends RestResponseBase {

    private ParkingPreferentialRuleDTO response;

    public ParkSetParkingPreferentialRuleRestResponse () {
    }

    public ParkingPreferentialRuleDTO getResponse() {
        return response;
    }

    public void setResponse(ParkingPreferentialRuleDTO response) {
        this.response = response;
    }
}
