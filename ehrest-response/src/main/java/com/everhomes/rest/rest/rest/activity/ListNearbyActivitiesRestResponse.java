// @formatter:off
// generated file: DO NOT EDIT
package com.everhomes.rest.rest.rest.activity;

import com.everhomes.rest.RestResponseBase;

import com.everhomes.rest.activity.ListNearbyActivitiesResponse;

public class ListNearbyActivitiesRestResponse extends RestResponseBase {

    private ListNearbyActivitiesResponse response;

    public ListNearbyActivitiesRestResponse () {
    }

    public ListNearbyActivitiesResponse getResponse() {
        return response;
    }

    public void setResponse(ListNearbyActivitiesResponse response) {
        this.response = response;
    }
}
