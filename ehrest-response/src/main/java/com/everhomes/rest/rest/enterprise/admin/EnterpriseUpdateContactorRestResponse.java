// @formatter:off
// generated file: DO NOT EDIT
package com.everhomes.rest.rest.enterprise.admin;

import com.everhomes.rest.RestResponseBase;

import com.everhomes.rest.enterprise.ListEnterpriseResponse;

public class EnterpriseUpdateContactorRestResponse extends RestResponseBase {

    private ListEnterpriseResponse response;

    public EnterpriseUpdateContactorRestResponse () {
    }

    public ListEnterpriseResponse getResponse() {
        return response;
    }

    public void setResponse(ListEnterpriseResponse response) {
        this.response = response;
    }
}
