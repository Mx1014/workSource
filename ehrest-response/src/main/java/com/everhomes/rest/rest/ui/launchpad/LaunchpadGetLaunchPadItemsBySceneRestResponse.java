// @formatter:off
// generated file: DO NOT EDIT
package com.everhomes.rest.rest.ui.launchpad;

import com.everhomes.rest.RestResponseBase;

import com.everhomes.rest.launchpad.GetLaunchPadItemsCommandResponse;

public class LaunchpadGetLaunchPadItemsBySceneRestResponse extends RestResponseBase {

    private GetLaunchPadItemsCommandResponse response;

    public LaunchpadGetLaunchPadItemsBySceneRestResponse () {
    }

    public GetLaunchPadItemsCommandResponse getResponse() {
        return response;
    }

    public void setResponse(GetLaunchPadItemsCommandResponse response) {
        this.response = response;
    }
}
