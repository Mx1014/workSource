...
update `eh_rentalv2_order_record` t1 right join `eh_rentalv2_orders` t2 on t1.`order_no` = t2.`order_no` set t1.order_id = t2.id,t1.status = IF(t2.status in (2,7,9,10,14,20),1,0);
