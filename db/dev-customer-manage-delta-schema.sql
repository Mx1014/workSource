...
--
-- 给eh_organization_owners增加一些字段
--
ALTER TABLE `eh_organization_owners`
  ADD COLUMN `registered_residence` VARCHAR(128) COMMENT '户口所在地',
  ADD COLUMN `organization_owner_type` VARCHAR(20) COMMENT '业主, 租户, 亲戚, 朋友',
  ADD COLUMN `gender` TINYINT COMMENT '男,女',
  ADD COLUMN `birthday` DATE COMMENT 'birthday',
  ADD COLUMN `marital_status` TINYINT COMMENT '未婚,已婚,离异,丧偶',
  ADD COLUMN `job` VARCHAR(10) COMMENT 'job',
  ADD COLUMN `company` TINYINT COMMENT 'company',
  ADD COLUMN `id_card_number` VARCHAR(18) COMMENT 'id card number',
  ADD COLUMN `avatar` VARCHAR(128) COMMENT 'avatar'

--
-- 创建eh_cars表,汽车管理的汽车表
--
CREATE TABLE `eh_cars` (
...
--
-- 创建eh_organization_owner与eh_address的多对多表
--
CREATE TABLE `eh_organization_owner_address` (
  `id` BIGINT NOT NULL AUTO_INCREMENT,
  `namespace_id` BIGINT NOT NULL DEFAULT 0,
  `organizaion_owner_id` BIGINT,
  `address_id` BIGINT,
  PRIMARY KEY (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;

--
-- 创建eh_organization_owner与eh_owner_car的多对多表
--
CREATE TABLE `eh_organization_owner_car` (
  `id` BIGINT NOT NULL AUTO_INCREMENT,
  `namespace_id` BIGINT NOT NULL DEFAULT 0,
  `organizaion_owner_id` BIGINT,
  `car_id` BIGINT,
  `primary_flag` TINYINT COMMENT '是否是是首要联系人, yes: 1, no: 0',
  PRIMARY KEY (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
...
-- 车辆管理中的附件上传记录表
--
CREATE TABLE `eh_car_attachments` (
  `id` BIGINT NOT NULL AUTO_INCREMENT,
  `namespace_id` BIGINT NOT NULL DEFAULT 0,
