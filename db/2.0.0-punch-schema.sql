...
ALTER TABLE `eh_punch_rules` ADD COLUMN `owner_type` VARCHAR(128) COMMENT 'owner resource(user/organization) type';
ALTER TABLE `eh_punch_rules` ADD COLUMN `owner_id` BIGINT  COMMENT 'owner resource(user/organization) id';
ALTER TABLE `eh_punch_rules` ADD COLUMN `location_rule_id` BIGINT  COMMENT 'fk:eh_punch_geopoints id';
ALTER TABLE `eh_punch_rules` ADD COLUMN `description` VARCHAR(256);
ALTER TABLE `eh_punch_rules` ADD KEY `fk_eh_punch_location_rules` (`location_rule_id`);
ALTER TABLE `eh_punch_rules` ADD CONSTRAINT `fk_eh_punch_geopoints_ibfk_1` FOREIGN KEY (`location_rule_id`) REFERENCES `eh_punch_location_rules`(`id`);
...
CREATE TABLE `eh_punch_rules` (
  ...
  PRIMARY KEY (`id`),
  KEY `fk_eh_punch_time_rules` (`time_rule_id`),
  CONSTRAINT `fk_eh_punch_rules_ibfk_1` FOREIGN KEY (`time_rule_id`) REFERENCES `eh_punch_time_rules` (`id`),
  KEY `fk_eh_punch_location_rules` (`location_rule_id`),
  CONSTRAINT `fk_eh_punch_rules_ibfk_2` FOREIGN KEY (`location_rule_id`) REFERENCES `eh_punch_location_rules` (`id`),
  KEY `fk_eh_punch_wifi_rules` (`wifi_rule_id`),
  CONSTRAINT `fk_eh_punch_rules_ibfk_3` FOREIGN KEY (`wifi_rule_id`) REFERENCES `eh_punch_wifi_rules` (`id`),
  KEY `fk_eh_punch_scheduling_rules` (`scheduling_rule_id`),
  CONSTRAINT `fk_eh_punch_rules_ibfk_4` FOREIGN KEY (`scheduling_rule_id`) REFERENCES `eh_punch_scheduling_rules` (`id`)
) ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 ;
