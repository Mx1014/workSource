...
CREATE TABLE `eh_organization_owners` (
  ...
  `org_owner_type_id` BIGINT COMMENT '业主类型id',
  `gender` TINYINT COMMENT '男,女',
...
CREATE TABLE `eh_organizations` (
  ...
  `group_type` VARCHAR(64) COMMENT 'enterprise, department, service_group',
  `create_time` DATETIME,
  `update_time` DATETIME,
...
CREATE TABLE `eh_punch_exception_requests` (
  ...
  `operate_time` DATETIME,
  `view_flag` TINYINT NOT NULL DEFAULT 1 COMMENT 'is view(0) not view(1)',
  `request_id` BIGINT NULL COMMENT 'approval request id',
