...
CREATE TABLE `eh_var_fields` (
  ...
  `name` BIGINT NOT NULL DEFAULT 0 COMMENT 'The field logic name, it map to the field in db',
  `display_name` BIGINT NOT NULL DEFAULT 0 COMMENT 'The field display name',
...
CREATE TABLE `eh_var_field_items` (
  `id` BIGINT NOT NULL COMMENT 'id for records',
  `module_name` VARCHAR(128) NOT NULL DEFAULT '' COMMENT 'the module which the field belong to',
  `field_name` BIGINT NOT NULL DEFAULT 0 COMMENT 'The field which the item belong to',
  `display_name` BIGINT NOT NULL DEFAULT 0 COMMENT 'The field display name',
  `default_order` INTEGER,
  `status` TINYINT NOT NULL DEFAULT 0 COMMENT '0: inactive, 1: waiting for approval, 2: active',
  `creator_uid` BIGINT,
...
CREATE TABLE `eh_var_field_group_scopes` (
  ...
  `module_name` VARCHAR(128) NOT NULL DEFAULT '' COMMENT 'the module which the field belong to',
  `group_id` BIGINT NOT NULL DEFAULT 0 COMMENT 'refer to eh_var_field_groups',
  `group_display_name` BIGINT COMMENT 'the group name, it will use the name in eh_var_field_groups if not defined',
  `default_order` INTEGER,
  `status` TINYINT NOT NULL DEFAULT 0 COMMENT '0: inactive, 1: waiting for approval, 2: active',
  `creator_uid` BIGINT,
  `create_time` DATETIME,
...
CREATE TABLE `eh_var_field_scopes` (
  ...
  `field_display_name` BIGINT COMMENT 'the field name, it will use the name in eh_var_fields if not defined',
  `default_order` INTEGER,
  `status` TINYINT NOT NULL DEFAULT 0 COMMENT '0: inactive, 1: waiting for approval, 2: active',
  `creator_uid` BIGINT,
  `create_time` DATETIME,
...
CREATE TABLE `eh_var_field_item_scopes` (
  ...
  `item_display_name` BIGINT COMMENT 'the field name, it will use the name in eh_var_field_items if not defined',
  `default_order` INTEGER,
  `status` TINYINT NOT NULL DEFAULT 0 COMMENT '0: inactive, 1: waiting for approval, 2: active',
  `creator_uid` BIGINT,
  `create_time` DATETIME,
...
CREATE TABLE `eh_enterprise_customers` (
  ...
  `level_item_id` BIGINT COMMENT '客户级别：枚举型、普通客户、重要客户、意向客户、已成交客户、其他, refer to the id of eh_var_field_items',
