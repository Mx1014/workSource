...
CREATE TABLE `eh_customer_statistics_daily` (
 `id` BIGINT NOT NULL,
  ...
  `new_customer_num` BIGINT COMMENT '新增客户增量',
  `registered_customer_num` BIGINT COMMENT '成交客户增量',
  `loss_customer_num` BIGINT COMMENT '流失客户增量',
  `history_customer_num` BIGINT COMMENT '历史客户增量',
  `delete_customer_num` BIGINT COMMENT '删除客户增量',
  `tracking_num` BIGINT COMMENT '跟进次数增量',
