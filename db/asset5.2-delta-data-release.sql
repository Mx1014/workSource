...
set @park_menu_parent_parent_id=16000000;-- select * from eh_web_menus where name='园区运营业务'
set @park_menu_parent_id=40000030; -- select * from eh_web_menus where name='物业管理' and type='park'
set @park_menu_path=CONCAT("/",@park_menu_parent_parent_id,"/",@park_menu_parent_id,"/",@park_menu_id);-- 如：/16000000/40000030/XXX
-- 普通公司后台菜单路径（三层）
set @organization_menu_id=(select max(id) + 3 from eh_web_menus);-- 如：16032200
set @organization_menu_parent_parent_id=16000000;-- select * from eh_web_menus where name='园区运营业务'
set @organization_menu_parent_id=40000030; -- select * from eh_web_menus where name='物业管理' and type='organization'
set @organization_menu_path=CONCAT("/",@organization_menu_parent_parent_id,"/",@organization_menu_parent_id,"/",@organization_menu_id);-- 如：/16000000/40000030/XXX
...
-- 园区（levle=3)
INSERT INTO `eh_web_menus`(`id`, `name`, `parent_id`, `icon_url`, `data_type`, `leaf_flag`, `status`, `path`, `type`, `sort_num`, `module_id`, `level`, `condition_type`, `category`, `config_type`) 
VALUES (@park_menu_id, '对公转账', @park_menu_parent_id, NULL, @data_type, 1, 2, @park_menu_path, 'park', 22, @module_id, 3, 'system', 'module', NULL);
-- 普通公司（levle=3)
INSERT INTO `eh_web_menus`(`id`, `name`, `parent_id`, `icon_url`, `data_type`, `leaf_flag`, `status`, `path`, `type`, `sort_num`, `module_id`, `level`, `condition_type`, `category`, `config_type`) 
VALUES (@organization_menu_id, '对公转账', @organization_menu_parent_id, NULL, @data_type, 1, 2, @organization_menu_path, 'organization', 22, @module_id, 3, 'system', 'module', NULL);
