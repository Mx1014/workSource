...
set @zuolin_menu_parent_parent_id=16000000;-- select * from eh_web_menus where name='业务模块管理' and parent_id=0
set @zuolin_menu_parent_id=16050000; -- select * from eh_web_menus where name='物业管控模块'
set @zuolin_menu_path=CONCAT("/",@zuolin_menu_parent_parent_id,"/",@zuolin_menu_parent_id,"/",@zuolin_menu_id);-- 如：/16000000/16030000/16032200
-- 园区后台菜单路径（两层）
set @park_menu_id=(select max(id) + 2 from eh_web_menus);-- 如：16032200
set @park_menu_parent_id=44000000; -- select * from eh_web_menus where name='租赁管理' and parent_id=0
set @park_menu_path=CONCAT("/",@park_menu_parent_id,"/",@park_menu_id);-- 如：/44000000/16032200
-- 普通公司后台菜单路径（两层）
set @organization_menu_id=(select max(id) + 3 from eh_web_menus);-- 如：16032200
set @organization_menu_parent_id=44000000; -- select * from eh_web_menus where name='租赁管理' and parent_id=0
set @organization_menu_path=CONCAT("/",@organization_menu_parent_id,"/",@organization_menu_id);-- 如：/44000000/16032200
...
VALUES (@zuolin_menu_id, '对公转账', @zuolin_menu_parent_id, NULL, @data_type, 1, 2, @zuolin_menu_path, 'zuolin', 22, @module_id, 3, 'system', 'module', NULL);
-- 园区（levle=2)
INSERT INTO `eh_web_menus`(`id`, `name`, `parent_id`, `icon_url`, `data_type`, `leaf_flag`, `status`, `path`, `type`, `sort_num`, `module_id`, `level`, `condition_type`, `category`, `config_type`) 
VALUES (@park_menu_id, '对公转账', @park_menu_parent_id, NULL, @data_type, 1, 2, @park_menu_path, 'park', 22, @module_id, 2, 'system', 'module', NULL);
-- 普通公司（levle=2)
INSERT INTO `eh_web_menus`(`id`, `name`, `parent_id`, `icon_url`, `data_type`, `leaf_flag`, `status`, `path`, `type`, `sort_num`, `module_id`, `level`, `condition_type`, `category`, `config_type`) 
VALUES (@organization_menu_id, '对公转账', @organization_menu_parent_id, NULL, @data_type, 1, 2, @organization_menu_path, 'organization', 22, @module_id, 2, 'system', 'module', NULL);
