...
VALUES (51000, '举报管理', 50000, NULL, '****', 0, 2, '/50000/51000', 'park', 510);

SET @acl_privileges = (SELECT MAX(id) FROM `eh_acl_privileges`);
INSERT INTO `eh_acl_privileges` (`id`, `app_id`, `name`, `description`, `tag`) 
VALUES ((@acl_privileges := @acl_privileges + 1), 0, '举报管理', '举报管理 管理员', NULL);

SET @web_menu_privilege_id = (SELECT MAX(id) FROM `eh_web_menu_privileges`);
INSERT INTO `eh_web_menu_privileges` (`id`, `privilege_id`, `menu_id`, `name`, `show_flag`, `status`, `discription`, `sort_num`) 
VALUES ((@web_menu_privilege_id := @web_menu_privilege_id + 1), @acl_privileges, 51000, '举报管理', 1, 1, '举报管理  管理员权限', 710); 
...
VALUES ((@eh_service_module_privileges_id := @eh_service_module_privileges_id + 1), '51000', '1', '@acl_privileges', NULL, '0', UTC_TIMESTAMP());

SET @eh_service_module_scopes_id = (SELECT MAX(id) FROM `eh_service_module_scopes`);
INSERT INTO `eh_service_module_scopes` (`id`, `namespace_id`, `module_id`, `module_name`, `owner_type`, `owner_id`, `default_order`, `apply_policy`)
VALUES ((@eh_service_module_scopes_id := @eh_service_module_scopes_id + 1), 999992, 51000, '举报管理', 'EhNamespaces', 1000000, NULL, 2);
