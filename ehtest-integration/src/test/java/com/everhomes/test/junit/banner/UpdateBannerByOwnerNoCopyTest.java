...
public class UpdateBannerByOwnerNoCopyTest extends BaseLoginAuthTestCase {
    @Before
    public void setUp() {
        super.setUp();
    }
     ...
     */
    @Test
    public void testUpdateBannerByOwner_testNoCopy() {
        ...
        assertEquals(result.get(0).getId(), ehBanners.get(0).getId());
        
        Integer count = context.selectCount().from(Tables.EH_BANNERS).fetchOne().into(Integer.class);
     ...
     */
    @Test
    public void testUpdateBannerByOwner_testUpdate() {
    	List<EhBanners> result = new ArrayList<>();
    	DSLContext context = dbProvider.getDslContext();
    	context.select().from(EH_BANNERS)
    	.fetch().map((r) -> {
    ...
    protected void initCustomData() {
    	String jsonFilePath = "data/json/create-banner-by-owner-create-customized-test-data.txt";
