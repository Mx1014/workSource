...
public class BatchUpdateOrganizationContactVisibleFlagTest extends BaseLoginAuthTestCase {
    ...
    @Test
    public void testBatchUpdateOrganizationContactVisibleFlag() {
        Integer namespaceId = 0;
        String userIdentifier = "12000000001";
        String plainTexPassword = "123456";
        logon(namespaceId, userIdentifier, plainTexPassword);
        
        String commandRelativeUri = "/org/batchUpdateOrganizationContactVisibleFlag";

        BatchUpdateOrganizationContactVisibleFlagCommand cmd = new BatchUpdateOrganizationContactVisibleFlagCommand();
        cmd.setOrganizationId(1000750L);
        List<String> contactTokens = new ArrayList<>();
        contactTokens.add("12000000001");
        contactTokens.add("12000000002");
        cmd.setContactTokens(contactTokens);
        ...
        context.select().from(Tables.EH_ORGANIZATION_MEMBERS).where(Tables.EH_ORGANIZATION_MEMBERS.ORGANIZATION_ID.eq(1000750L).and(Tables.EH_ORGANIZATION_MEMBERS.CONTACT_TOKEN.in("12000000001","12000000002")).and(Tables.EH_ORGANIZATION_MEMBERS.VISIBLE_FLAG.eq(VisibleFlag.HIDE.getCode()))).fetch().map((r) -> {
