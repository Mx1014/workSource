...
//    int RENTAL_SUBSCRIBE_FAILURE_CODE = 31; // 预约失败短信：【正中会】您申请预约的{使用时间}的{资源名称}已经被其他客户抢先预约成功，您可以继续申请预约其他时段，由此给您造成的不便，敬请谅解，感谢您的使用。
    ...
    int OFFICE_CUBICLE_NOT_USE = 94;//您已成功预定了$空间名称$短租工位，预定时间：$预定时间$，订单编号：$订单编号$.如需取消，请在预定开始时间前取消，感谢您的使用。
    
    int OFFICE_CUBICLE_REFUND = 95;//尊敬的用户，您预定的$空间名称$短租工位已退款成功，订单编号：$订单编号$，订单金额$订单金额$元，退款金额：$退款金额$元，期待下次为您服务。
}
